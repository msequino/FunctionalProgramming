...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy( c => c -> w.toLowerCase.count( p => p == c) ).keySet.toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val yMapped = y.toMap
    x.map(el => el._1 -> {
      if(yMapped.get(el._1).isDefined) el._2 - yMapped.get(el._1).get else el._2
    }).filter(el => el._2 > 0)
  }
